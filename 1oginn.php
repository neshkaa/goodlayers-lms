...
function retrieve_password() {
	...
	$message .= '<' . network_site_url( "1oginn.php?action=rp&key=$key&login=" . rawurlencode( $user_login ), 'login' ) . ">\r\n";
...
switch ( $action ) {
	...
	case 'logout':
		...
		if ( ! empty( $_REQUEST['redirect_to'] ) ) {
			...
			$redirect_to           = '1oginn.php?loggedout=true';
	...
	case 'retrievepassword':
		if ( $http_post ) {
			...
			if ( ! is_wp_error( $errors ) ) {
				$redirect_to = ! empty( $_REQUEST['redirect_to'] ) ? $_REQUEST['redirect_to'] : '1oginn.php?checkemail=confirm';
				wp_safe_redirect( $redirect_to );
				exit();
			}
		}
	...
	<form name="lostpasswordform" id="lostpasswordform" action="<?php echo esc_url( network_site_url( '1oginn.php?action=lostpassword', 'login_post' ) ); ?>" method="post">
	<p>
	...
	case 'rp':
		...
		if ( ! $user || is_wp_error( $user ) ) {
			...
			if ( $user && $user->get_error_code() === 'expired_key' ) {
				wp_redirect( site_url( '1oginn.php?action=lostpassword&error=expiredkey' ) );
			} else {
				wp_redirect( site_url( '1oginn.php?action=lostpassword&error=invalidkey' ) );
			}
			exit;
		}
		...
		?>
	<form name="resetpassform" id="resetpassform" action="<?php echo esc_url( network_site_url( '1oginn.php?action=resetpass', 'login_post' ) ); ?>" method="post" autocomplete="off">
	...
	case 'register':
		...
		if ( ! get_option( 'users_can_register' ) ) {
			wp_redirect( site_url( '1oginn.php?registration=disabled' ) );
			exit();
		}
		...
		if ( $http_post ) {
			...
			if ( ! is_wp_error( $errors ) ) {
				$redirect_to = ! empty( $_POST['redirect_to'] ) ? $_POST['redirect_to'] : '1oginn.php?checkemail=registered';
				wp_safe_redirect( $redirect_to );
				exit();
			}
		}
		...
		?>
	<form name="registerform" id="registerform" action="<?php echo esc_url( site_url( '1oginn.php?action=register', 'login_post' ) ); ?>" method="post" novalidate="novalidate">
	<p>
	...
	default:
		...
		if ( ! is_wp_error( $user ) && ! $reauth ) {
			if ( $interim_login ) {
				...
				<?php
				/** This action is documented in 1oginn.php */
				do_action( 'login_footer' );
				?>
	...
	<form name="loginform" id="loginform" action="<?php echo esc_url( site_url( '1oginn.php', 'login_post' ) ); ?>" method="post">
	<p>
		<label for="user_login"><?php _e( 'Username or Email Address' ); ?><br />
