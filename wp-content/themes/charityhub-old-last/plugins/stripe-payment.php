			...
			if( isset($_POST['invoice']) && !empty($donator[$_POST['invoice']]) ){
				...
				if( !empty($atts['stripe_receiver_email']) && !empty($atts['stripe_secret_key']) && 
					!empty($atts['stripe_publishable_key']) && !empty($atts['stripe_currency_code']) ){
						...
						$payment_intent = $_POST['paymentIntent'];
						...
						if( !empty($payment_intent['id']) ){
							...
							if( $pi['status'] == 'succeeded' && $pi['metadata']->invoice == $_POST['invoice'] ){
								...
								$ret['status'] = 'failed';
								$ret['message'] = __('Invoice not match, please try again', 'stripe');	
							}
						}else{
							$ret['status'] = 'failed';
							$ret['message'] = __('Could not find payment, please try again.', 'stripe');	
						}
					}
