...
?>
	...
	<?php }else if( is_archive() || is_search() ){
		if( is_search() ){
			...
			$title = esc_html__('Events', 'gdlr_translate');
			$caption = '';
		}else if( is_tax('event-category')){ 			
			$title = tribe_meta_event_category_name();		
			$caption = '';
		}else if( is_category() || is_tax('portfolio_category') || is_tax('product_cat') || is_tax('cause_category') || is_tax('cause_tax') ){
