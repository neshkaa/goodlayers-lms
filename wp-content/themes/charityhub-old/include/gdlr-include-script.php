	...
	if( !function_exists('gdlr_regiser_use_script') ){
		function gdlr_regiser_use_script($array){	
			...
			if( empty($multisite) || $multisite == 1 ){
				$filemtime = filemtime(GDLR_LOCAL_PATH . '/stylesheet/style-custom.css');
				$array['style']['style-custom'] = GDLR_PATH . '/stylesheet/style-custom.css?' . $filemtime;	
			}else{
				$filemtime = filemtime(GDLR_LOCAL_PATH . '/stylesheet/style-custom' . $multisite . '.css');
				$array['style']['style-custom'] = GDLR_PATH . '/stylesheet/style-custom' . $multisite . '.css?' . $filemtime;			
			}
