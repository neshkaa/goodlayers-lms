<?php
	...
	*/
	if( !function_exists('gdlr_lms_preview_lightbox_form') ){
		function gdlr_lms_preview_lightbox_form($content = '', $slug = ''){
?>
<div class="gdlr-lms-lightbox-container lecture-preview <?php echo esc_attr($slug); ?>" data-return="parent">
	<div class="gdlr-lms-lightbox-close"><i class="fa fa-remove icon-remove"></i></div>
	<?php echo gdlr_lms_text_filter($content); ?>
</div>
<?php
		}
	}
	
	if( !function_exists('gdlr_lms_sign_in_lightbox_form') ){
		function gdlr_lms_sign_in_lightbox_form(){
?>
<div class="gdlr-lms-lightbox-container login-form">
	<div class="gdlr-lms-lightbox-close"><i class="fa fa-remove icon-remove"></i></div>

	<h3 class="gdlr-lms-lightbox-title"><?php esc_html_e('Моля първо влез в акаунта си', 'gdlr-lms'); ?></h3>
	<form class="gdlr-lms-form gdlr-lms-lightbox-form" id="loginform" method="post" action="<?php echo esc_url(site_url('wp-login.php', 'login_post')); ?>">
		<p class="gdlr-lms-half-left">
			<span><?php esc_html_e('Потребителско име', 'gdlr-lms'); ?></span>
			<input type="text" name="log" />
		</p>
		<p class="gdlr-lms-half-right">
			 <span><?php esc_html_e('Парола', 'gdlr-lms'); ?></span>
			 <input type="password" name="pwd" />
		</p>
		<div class="clear"></div>
		<p class="gdlr-lms-lost-password" >
			<?php $login_url = empty($_GET['login'])? home_url(): $_GET['login']; ?>
			<a href="<?php echo wp_lostpassword_url(); ?>" ><?php esc_html_e('Забравена парола?','gdlr-lms'); ?></a>
		</p>
		<p>
			...
			<input type="submit" name="wp-submit" class="gdlr-lms-button" value="<?php esc_html_e('Вход', 'gdlr-lms'); ?>" />
		</p>
	</form>
	<h3 class="gdlr-lms-lightbox-title second-section"><?php esc_html_e('Не си регистриран?', 'gdlr-lms'); ?></h3>
	<div class="gdlr-lms-lightbox-description"><?php esc_html_e('Регистрирай се безплатно и получи достъп до всички допълнителни образователни материали.', 'gdlr-lms'); ?></div>
	<a class="gdlr-lms-button blue" href="<?php echo esc_url(add_query_arg('register', get_the_ID(), home_url())); ?>"><?php esc_html_e('Регистрация', 'gdlr-lms'); ?></a>
</div>
<?php
		}
	}

	if( !function_exists('gdlr_lms_quiz_timeout_form') ){
		function gdlr_lms_quiz_timeout_form($page = 0){
?>
<div class="gdlr-lms-lightbox-container quiz-timeout-form">
	<h3 class="gdlr-lms-lightbox-title"><?php esc_html_e('Time out!', 'gdlr-lms'); ?></h3>
	<div class="gdlr-lms-lightbox-quiz-timeout-content">
		<?php if( !empty($page) ){ ?>
		<div class="quiz-timeout-content"><?php
			esc_html_e('This part is timeout! press the button below to skip to next part', 'gdlr-lms');
		?></div>
		<a class="gdlr-lms-button blue submit-quiz-form" href="<?php echo esc_url(add_query_arg(array('course_type'=>'quiz', 'course_page'=> $page))); ?>" ><?php
			esc_html_e('Continue the quiz', 'gdlr-lms');
		?></a>
		<?php }else{ ?>
		<div class="quiz-timeout-content"><?php
			esc_html_e('This part is timeout! press the button to submit the quiz', 'gdlr-lms');
		?></div>
		<a class="gdlr-lms-button blue submit-quiz-timeout-form" ><?php
			esc_html_e('Submit the quiz', 'gdlr-lms');
		?></a>
		<?php } ?>
	</div>
...
<?php
	...
	if( !function_exists('gdlr_lms_section_quiz_timeout_form') ){
...
<?php
	...
	if( !function_exists('gdlr_lms_rating_form') ){
		function gdlr_lms_rating_form($course_id){
?>
<div class="gdlr-lms-lightbox-container rating-form">
	<div class="gdlr-lms-lightbox-close"><i class="fa fa-remove icon-remove"></i></div>

	<h3 class="gdlr-lms-lightbox-title"><?php echo esc_html__('Rate the course', 'gdlr-lms'); ?></h3>
	...
	<form class="gdlr-lms-form gdlr-lms-lightbox-form" method="post" action="<?php echo esc_url(add_query_arg('type', 'attended-courses')); ?>">
		...
		<input type="submit" class="gdlr-lms-button cyan" value="<?php echo esc_attr__('Rate !', 'gdlr-lms'); ?>" />
	</form>
...
<?php
	...
	if( !function_exists('gdlr_lms_payment_option_form') ){
...
<div class="gdlr-lms-lightbox-container payment-option-form">
	...
	<div class="gdlr-lms-payment-option-wrapper gdlr-lms-left">
		<div class="gdlr-lms-payment-option-inner">
			<h4 class="gdlr-lms-payment-option-head"><?php esc_html_e('Pay now.', 'gdlr-lms'); ?></h4>
			<a class="gdlr-lms-button cyan" data-rel="gdlr-lms-lightbox3" data-lb-open="buy-form" ><?php esc_html_e('Pay Now', 'gdlr-lms'); ?></a>
			<div class="gdlr-lms-payment-option-description"><?php
				esc_html_e('* You\'re not required to submit evidence of payment after you pay via PayPal.','gdlr-lms');
			?></div>
		</div>
	</div>
	<div class="gdlr-lms-payment-option-or"><?php esc_html_e('OR', 'gdlr-lms'); ?></div>
	<div class="gdlr-lms-payment-option-wrapper gdlr-lms-right">
		<div class="gdlr-lms-payment-option-inner">
			<h4 class="gdlr-lms-payment-option-head"><?php esc_html_e('Submit evidence of payment.', 'gdlr-lms'); ?></h4>
			<a class="gdlr-lms-button blue" data-rel="gdlr-lms-lightbox3" data-lb-open="evidence-form" ><?php esc_html_e('Continue', 'gdlr-lms'); ?></a>
			<div class="gdlr-lms-payment-option-description"><?php
				esc_html_e('* Noted that you must pay via method we provided before sumitting evidence.','gdlr-lms');
			?></div>
		</div>
	</div>
</div>
<?php
	...
	if( !function_exists('gdlr_lms_evidence_lightbox_form') ){
...
<div class="gdlr-lms-lightbox-container evidence-form">
	...
	?>
	<h3 class="gdlr-lms-lightbox-title"><?php echo gdlr_lms_text_filter($fix_val['title']); ?></h3>
	<form class="gdlr-lms-form gdlr-lms-lightbox-form" method="post" enctype="multipart/form-data" action="<?php echo esc_url(add_query_arg($_GET)); ?>">
		<p>
			<span><?php esc_html_e('Additional Note', 'gdlr-lms'); ?></span>
			<textarea class="full-note" name="additional-note" ><?php echo gdlr_lms_text_filter($fix_val['additional_note']); ?></textarea>
		</p>
		<p>
			<span><?php esc_html_e('Select Attachment', 'gdlr-lms'); ?></span>
			<input type="file" name="attachment" />
		</p>
		<p>
			<span><?php esc_html_e('Total Price', 'gdlr-lms'); ?></span>
		...
		<p>
			...
			<input type="hidden" name="invoice" value="<?php echo esc_attr($fix_val['id']); ?>">
			<input type="submit" class="gdlr-lms-button" value="<?php esc_html_e('Submit', 'gdlr-lms'); ?>" />
		</p>
	</form>
</div>
<?php
	...
	if( !function_exists('gdlr_lms_purchase_lightbox_form') ){
...
?>
<div class="gdlr-lms-lightbox-container <?php echo esc_attr($type); ?>-form">
	<?php
		if($close == 'close'){
			echo '<div class="gdlr-lms-lightbox-close"><i class="fa fa-remove icon-remove"></i></div>';
		}else if($close != 'none'){
	...
	<h3 class="gdlr-lms-lightbox-title"><?php echo gdlr_lms_text_filter($fix_val['title']); ?></h3>
	<form class="gdlr-lms-form gdlr-lms-lightbox-form <?php echo esc_attr($fix_val['form-class']); ?>" method="post" <?php
		if( $type == 'buy' ) echo 'action="' . $lms_paypal['url'] . '"'
	?> data-ajax="<?php echo admin_url('admin-ajax.php'); ?>?lang=<?php echo substr(get_locale(), 0, 2); ?>">
		<p class="gdlr-lms-half-left">
			<span><?php esc_html_e('Name', 'gdlr-lms'); ?></span>
			<input type="text" name="first_name" value="<?php echo esc_attr($fix_val['first_name']); ?>" <?php echo gdlr_lms_text_filter($disabled); ?> />
		</p>
		<p class="gdlr-lms-half-right">
			 <span><?php esc_html_e('Lastname', 'gdlr-lms'); ?></span>
			 <input type="text" name="last_name" value="<?php echo esc_attr($fix_val['last_name']); ?>" <?php echo gdlr_lms_text_filter($disabled); ?> />
		</p>
		<div class="clear"></div>
		<p class="gdlr-lms-half-left">
			<span><?php esc_html_e('Email', 'gdlr-lms'); ?></span>
			<input type="text" name="email" value="<?php echo esc_attr($fix_val['email']); ?>" <?php echo gdlr_lms_text_filter($disabled); ?> />
		</p>
		<p class="gdlr-lms-half-right">
			 <span><?php esc_html_e('Phone', 'gdlr-lms'); ?></span>
			 <input type="text" name="phone" value="<?php echo esc_attr($fix_val['phone']); ?>" <?php echo gdlr_lms_text_filter($disabled); ?> />
		</p>
		<div class="clear"></div>
		<p class="gdlr-lms-half-left">
			<span><?php esc_html_e('Address', 'gdlr-lms'); ?></span>
			<textarea name="address" <?php echo gdlr_lms_text_filter($disabled); ?>><?php echo gdlr_lms_text_filter($fix_val['address']); ?></textarea>
		</p>
		<p class="gdlr-lms-half-right">
			<span><?php esc_html_e('Additional Note', 'gdlr-lms'); ?></span>
			<textarea name="additional-note" <?php echo gdlr_lms_text_filter($disabled); ?>><?php echo gdlr_lms_text_filter($fix_val['additional_note']); ?></textarea>
		</p>
		<div class="clear"></div>
		<p class="gdlr-lms-half-left">
			<?php 
				$amount_disabled = ($disabled == 'disabled' || $course_option['online-course'] == 'enable')? 'disabled': ''; 
				if( $amount_disabled == 'disabled' ){
					echo '<input type="hidden" name="quantity" value="' . esc_attr($fix_val['amount']) . '" >';
				}else{
					echo '<span>' . esc_html__('Amount', 'gdlr-lms') . '</span>';
					echo '<input type="text" name="quantity" value="' . esc_attr($fix_val['amount']) . '" />';
					echo '</p>';
					echo '<p class="gdlr-lms-half-right">';
				}
			...
			?>
			 ...
			 <input type="hidden" class="price-one" name="amount" value="<?php echo esc_attr($price); ?>" />
			 <input type="hidden" class="format" value="<?php echo esc_attr($lms_money_format); ?>" />
		</p>
		<div class="clear"></div>
		<div class="gdlr-lms-coupon-full-width">
			<span class="gdlr-lms-coupon-head"><?php esc_html_e('Coupon Code', 'gdlr-lms'); ?>
			...
			<input type="hidden" class="discount-amount" name="discount_amount" value="<?php echo esc_attr($fix_val['coupon-discount']); ?>" />
			<input type="hidden" class="coupon-amount" name="coupon-amount" value="" />
			<input type="hidden" class="coupon-type" name="coupon-type" value="" />
			<input type="text" class="gdlr-lms-coupon-code" name="gdlr-lms-coupon-code" value="<?php echo esc_attr($fix_val['coupon']); ?>" />
		</div>
		<?php 
			if( $type == "buy" ){ 
		...
		?>
		<p>
			<div class="gdlr-lms-notice"><?php esc_html_e('notice', 'gdlr-lms'); ?></div>
			<div class="gdlr-lms-loading"><?php esc_html_e('loading', 'gdlr-lms'); ?></div>
			<input type="hidden" name="rememberme"  value="forever" />
			<input type="hidden" name="course_id"  value="<?php echo esc_attr($fix_val['course-id']); ?>" />
			<input type="hidden" name="course_code"  value="<?php echo esc_attr($course_option['course-code']); ?>" />
			<input type="hidden" name="student_id"  value="<?php echo esc_attr($current_user->data->ID); ?>" />
			<input type="hidden" name="action" value="<?php 
				echo empty($fix_val['form-class'])? 'gdlr_lms_form_purchase' : 'gdlr_lms_form_price_validate';
			?>" />
			<input type="hidden" name="action_type" value="<?php echo esc_attr($type); ?>" />
			<input type="hidden" name="charset" value="utf-8"> 
			<input type="hidden" name="return" value="<?php echo add_query_arg(array('payment_complete'=>''), $fix_val['return']); ?>">
			<?php if($type == "buy"){ ?>
				<input type="hidden" name="cmd" value="_xclick">
				<input type="hidden" name="invoice" value="<?php echo esc_attr($fix_val['id']); ?>">
				<input type="hidden" name="business" value="<?php echo esc_attr($lms_paypal['recipient']); ?>">
				<input type="hidden" name="item_name" value="<?php echo esc_attr($fix_val['title']); ?>" />
				<input type="hidden" name="currency_code" value="<?php echo esc_attr($lms_paypal['currency_code']); ?>" />
			...
			<input type="submit" class="gdlr-lms-button" value="<?php
				echo ($type == 'book')? esc_html__('Book Now!', 'gdlr-lms'): esc_html__('Pay Now!', 'gdlr-lms');
			?>" />
		</p>
	</form>
</div>
<?php
	...
	if( !function_exists('gdlr_lms_coupon_discount') ){
		function gdlr_lms_coupon_discount($coupon_id, $course_id){
			...
			if(empty($coupon_id) || empty($course_id)){
				$ret['status'] = 'failed';
				$ret['message'] = esc_html__('An error occurs, please try again after refreshing the page.');
			}else{
				$sql  = "SELECT DISTINCT post_id FROM {$wpdb->postmeta} wpostmeta ";
				$sql .= $wpdb->prepare("WHERE meta_key = 'gdlr-coupon-code' AND meta_value = %s ", $coupon_id);
				...
				if( empty($query) ){
					...
					if( !empty($post_option) ){
							...
							if( $coupon_amount == -1 || $coupon_amount > $count ){
								if( empty($post_option['specify-course']) ){
									...
									if( in_array($course_id, $specify_course) ){
										...
										$ret['status'] = 'failed';
										$ret['message'] = esc_html__('This coupon isn\'t valid for this course', 'gdlr-lms');
									}
								}
							}else{
								$ret['status'] = 'failed';
								$ret['message'] = esc_html__('This coupon code has been used up', 'gdlr-lms');
							}
						}else{
							$ret['status'] = 'failed';
							$ret['message'] = esc_html__('Coupon has been expired', 'gdlr-lms');
						}
						if( $ret['status'] == 'success' ){
							...
							$ret['message'] = sprintf(esc_html__('You got %s discount', 'gdlr-lms'), $discount_text);							
						}
					}else{
						$ret['status'] = 'failed';
					}
	...
	if( !function_exists('gdlr_lms_check_coupon_code') ){
	...
	if( !function_exists('gdlr_lms_form_price_validate') ){
		function gdlr_lms_form_price_validate(){
			...
			if( abs($course_options['price-one'] - floatval($_POST['amount'])) > 0.00001 || $_POST['discount_amount'] != $coupon_discount ){
				$ret['status'] = 'failed';
				$ret['message'] = esc_html__('An error is occurred, please refresh the page to try this again.', 'gdlr-lms');
			}else{
				$ret['id'] = $_POST['invoice'];
				$ret['status'] = 'success';
				if( empty($_POST['payment-method']) || $_POST['payment-method'] == 'paypal' ){
					$ret['message'] = esc_html__('Redirecting to paypal', 'gdlr-lms');
					$ret['redirect'] = true;
					$ret['id'] = current_time('dmY') . $ret['id'];
				}else if( $_POST['payment-method'] == 'stripe' ){
					$ret['message'] = esc_html__('Redirecting to stripe payment', 'gdlr-lms');
					$ret['redirect'] = add_query_arg(array('payment-method'=> 'stripe', 'invoice'=>$ret['id']), home_url());
				}else if( $_POST['payment-method'] == 'paymill' ){
					$ret['message'] = esc_html__('Redirecting to paymill payment', 'gdlr-lms');
					$ret['redirect'] = add_query_arg(array('payment-method'=> 'paymill', 'invoice'=>$ret['id']), home_url());
				}else if( $_POST['payment-method'] == 'authorize' ){
					$ret['message'] = esc_html__('Redirecting to authorize payment', 'gdlr-lms');
					$ret['redirect'] = add_query_arg(array('payment-method'=> 'authorize', 'invoice'=>$ret['id']), home_url());
				}else if( $_POST['payment-method'] == 'braintree' ){
					$ret['message'] = esc_html__('Booking complete, redirecting to braintree payment', 'gdlr-lms');
					$ret['redirect'] = add_query_arg(array('payment-method'=> 'braintree', 'invoice'=>$ret['id']), home_url());
				}else if( $_POST['payment-method'] == 'cloud' ){
					$ret['message'] = esc_html__('Booking complete, proceed to cloud payment', 'gdlr-lms');
					$ret['payment'] = 'cloud';
					$ret['ok_button'] = esc_html__('Ok', 'gdlr-lms');
					$ret['payment_failed_text'] = esc_html__('Payment Failed', 'gdlr-lms');
					$ret['data'] = array(
						'publicId' => $gdlr_lms_option['cloud-public-id'],
						'description' => '"' . get_the_title($_POST['course_id']) . '" ' . esc_html__('couse purchasing', 'gdlr-lms'),
	...
	if( !function_exists('gdlr_lms_form_purchase') ){
		function gdlr_lms_form_purchase(){
			...
			if(wp_verify_nonce($_POST['gdlr_lms_purchase_form'], 'gdlr_lms_purchase_form')){
				...
				if( empty($_POST['first_name']) || empty($_POST['last_name']) || empty($_POST['email']) ){
					$ret['status'] = 'failed';
					$ret['message'] = esc_html__('Please fill all required fields.', 'gdlr-lms');

				}else if( gdlr_lms_payment_row_exists($_POST['course_id'], $_POST['student_id']) ){
					$ret['status'] = 'failed';
					$ret['message'] = esc_html__('You already booked this course, please proceed the payment via your profile page.', 'gdlr-lms');
					...
					if( (abs($course_options['price-one'] - floatval($_POST['amount'])) > 0.00001) || 
						...
						if( !empty($course_options['max-seat']) && $course_options['online-course'] == 'disable' &&
							...
							$ret['status'] = 'failed';
							$ret['message'] = esc_html__('This course is already full or the available seat is not enough, please try again later.', 'gdlr-lms');
						}else{
							...
							if( $result > 0 ){
								...
								$ret['status'] = 'success';
								if( $course_price == 0 ){
									$ret['message'] = esc_html__('Purchasing Complete', 'gdlr-lms');
									$ret['redirect'] = remove_query_arg('payment_complete', $_POST['return']);
								}else{
									if( $_POST['action_type'] == 'book' ){
										$ret['message'] = esc_html__('Booking complete', 'gdlr-lms');
										$ret['redirect'] = get_permalink($_POST['course_id']);
									}else{
										if( empty($_POST['payment-method']) || $_POST['payment-method'] == 'paypal' ){
											$ret['message'] = esc_html__('Booking complete, redirecting to paypal', 'gdlr-lms');
											$ret['redirect'] = true;
											$ret['id'] = current_time('dmY') . $ret['id'];
										}else if( $_POST['payment-method'] == 'stripe' ){
											$ret['message'] = esc_html__('Booking complete, redirecting to stripe payment', 'gdlr-lms');
											$ret['redirect'] = add_query_arg(array('payment-method'=> 'stripe', 'invoice'=>$ret['id']), home_url());
										}else if( $_POST['payment-method'] == 'paymill' ){
											$ret['message'] = esc_html__('Booking complete, redirecting to paymill payment', 'gdlr-lms');
											$ret['redirect'] = add_query_arg(array('payment-method'=> 'paymill', 'invoice'=>$ret['id']), home_url());
										}else if( $_POST['payment-method'] == 'authorize' ){
											$ret['message'] = esc_html__('Booking complete, redirecting to authorize payment', 'gdlr-lms');
											$ret['redirect'] = add_query_arg(array('payment-method'=> 'authorize', 'invoice'=>$ret['id']), home_url());
										}else if( $_POST['payment-method'] == 'braintree' ){
											$ret['message'] = esc_html__('Booking complete, redirecting to braintree payment', 'gdlr-lms');
											$ret['redirect'] = add_query_arg(array('payment-method'=> 'braintree', 'invoice'=>$ret['id']), home_url());
								...
								$ret['status'] = 'failed';
								$ret['message'] = esc_html__('Transaction error, please contact the administrator', 'gdlr-lms');
							}
						}
					}
				}
				...
				$ret['status'] = 'failed';
				$ret['message'] = esc_html__('Session expired, please refresh the page and try this again', 'gdlr-lms');
			}
	...
	if( !function_exists('gdlr_lms_delete_student') ){
		function gdlr_lms_delete_student(){	
			global $wpdb;

			$sql  = 'SELECT * FROM ' . $wpdb->prefix . 'gdlrpayment ';
			$sql .= $wpdb->prepare('WHERE id = %d ', $_POST['id']);
	...
	if( !function_exists('gdlr_lms_cancel_booking') ){
		function gdlr_lms_cancel_booking(){
			global $wpdb;

			$sql  = 'SELECT * FROM ' . $wpdb->prefix . 'gdlrpayment ';
			$sql .= $wpdb->prepare('WHERE id = %d AND ', $_POST['id']);
