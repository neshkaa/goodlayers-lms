...
if( !function_exists('gdlr_paypal_form') ){
	function gdlr_paypal_form($atts){
		extract( shortcode_atts(array('user'=>'',
			...
			'val_1'=>'5', 'val_2'=>'10', 'val_3'=>'20',
			'currency_format'=>'', 'currency_code'=>'лв', 'Donation Button Popup',
...
pll_e('You are donating to :','gdlr_translate'); echo __(' <span>' . get_the_title() . '</span>');
						 ?></h3>
...
<form class="gdlr-paypal-form" action="<?php echo $action; ?>" method="post" data-ajax="<?php echo AJAX_URL; ?>" >
		<div class="gdlr-paypal-amount-wrapper">
				<span class="gdlr-head"><?php
						...
						echo __('Каква сума искате да дарите?', 'gdlr_translate');
						} else {
				...
				<input type="text" class="custom-amount" data-default="<?php
					pll_e ('Or your amount', 'gdlr_translate'); echo __('(' . $currency_code . ')');
						?>" />
				...
				<div class="gdlr-recurring-payment-wrapper">
						...
						if (pll_current_language() == 'bg'){
						echo __('Искате ли да правите редовно дарение?', 'gdlr_translate');
						} else {
						...
						if (pll_current_language() == 'bg'){
						echo __('Искам да направя', 'gdlr_translate');
						} else {
						echo __('I would like to make ', 'gdlr_translate');
						} ?></span>

						<select name="t3" class="gdlr-recurring-option" >
								<option value="0"><?php if (pll_current_language() == 'bg'){
								_e('еднократно', 'gdlr_translate');
								} else {
								_e('a one time', 'gdlr_translate');} ?></option>
								<option value="W"><?php if (pll_current_language() == 'bg'){
								_e('ежеседмично', 'gdlr_translate');
								} else {
								_e('weekly', 'gdlr_translate');} ?></option>
								<option value="M"><?php if (pll_current_language() == 'bg'){
								_e('ежемесечно', 'gdlr_translate');
								} else {
								_e('monthly', 'gdlr_translate');} ?></option>
								<option value="Y"><?php if (pll_current_language() == 'bg'){
								_e('ежегодно', 'gdlr_translate');
								} else {
								_e('yearly', 'gdlr_translate');} ?></option>
						</select>
						<span class="gdlr-subhead" ><?php

						if (pll_current_language() == 'bg'){
						echo __('дарение', 'gdlr_translate');
						} else {
		...
		<div class="gdlr-paypal-fields">
				...
				<div class="six columns"><span class="gdlr-head"><?php
						...
						if (pll_current_language() == 'bg'){
						echo __('Фамилия *', 'gdlr_translate');
						} else {
				...
				<div class="six columns"><span class="gdlr-head"><?php
						...
						if (pll_current_language() == 'bg'){
						echo __('Телефон', 'gdlr_translate');
						} else {
				...
				<div class="six columns"><span class="gdlr-head"><?php
						...
						if (pll_current_language() == 'bg'){
						echo __('Съобщение', 'gdlr_translate');
						} else {
				echo __('Additional Note', 'gdlr_translate');
						...
						<textarea name="gdlr-additional-note"></textarea>
							...
							<a class = "tlc-policy" style="font-size:0.8em; float:right; margin-right:5%;" href = "<?php
				if (pll_current_language() == 'bg'){
				echo __ ('/privacy-policy">Политика за поверителност');
				} else {
		echo __('/en/privacy-policy-2/" >Privacy Policy');
				} ?> </a>
				<div class="clear"></div>
		</div>
		...
		<?php
				...
				echo '<a href="https://www.paypal.com/donate?hosted_button_id=PR8WGK8WSHF5Y"><img src="' . get_template_directory_uri() . '/images/paypal.png" alt="pp"</a>';
		?>
		...
		<div class="gdlr-notice email-invalid" ><?php
				...
				if (pll_current_language() == 'bg'){
				echo __ ('Невалиден email адрес ', 'gdlr_translate');
				} else {
		...
		<div class="gdlr-notice require-field" ><?php
				...
				if (pll_current_language() == 'bg'){
				echo __ ('Моля попълнете всички задължителни полета', 'gdlr_translate');
				} else {
		...
		<input type="submit" style="font-size:.9em;" value="<?php if (pll_current_language() == 'bg'){
				echo __ ('Дарявам', 'gdlr_translate');
				} else {
				_e('Donate', 'gdlr_translate');} ?>" >
</form>
</div>
<!-- recurring-1 -->
...
if( strcmp ($res, "VERIFIED") == 0 ) {
		...
		update_option('gdlr_paypal_debug', array(
				'num' => $num,
				'record' => $record[$num],
				'records' => $record,
		));
		if( empty($record[$num]['post-id']) ) return;
		// save the data beforehand
		...
		if( !empty($temp_option) && empty($mail_status) ){
		...
		if( empty($mail_status) || $mail_status == 'failed' ){
				wp_mail($record[$num]['email'], __('Thank you for your donation', 'gdlr_translate'), $message, $headers );
		}
		...
		if( empty($notify_status) || $notify_status == 'failed' ){
				wp_mail($_POST['receiver_email'], __('You received a new donation', 'gdlr_translate'), $message, $headers );
		}
...
add_action('init', function() {
  pll_register_string('You are donating to :', 'You are donating to :', 'Donation Button Popup');
});

add_action('init', function() {
  pll_register_string('Or your amount', 'Or your amount', 'Donation Button Popup');
});

add_action('init', function() {
  pll_register_string('лв', 'лв', 'Donation Button Popup');
});
