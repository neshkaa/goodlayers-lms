...
use pxlrbt\Cf7Cleverreach\Vendor\GuzzleHttp\Psr7\LazyOpenStream;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function createRejection(EasyHandle $easy, array $ctx)
    {
        ...
        if ($easy->onHeadersException) {
            return \pxlrbt\Cf7Cleverreach\Vendor\GuzzleHttp\Promise\rejection_for(new RequestException('An error was encountered during the on_headers event', $easy->request, $easy->response, $easy->onHeadersException, $ctx));
        }
        ...
        return \pxlrbt\Cf7Cleverreach\Vendor\GuzzleHttp\Promise\rejection_for($error);
    }
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf)
    {
        ...
        if (isset($options['sink'])) {
            $sink = $options['sink'];
            if (!\is_string($sink)) {
                $sink = \pxlrbt\Cf7Cleverreach\Vendor\GuzzleHttp\Psr7\stream_for($sink);
        ...
        if (isset($options['proxy'])) {
            if (!\is_array($options['proxy'])) {
                ...
                if (isset($options['proxy'][$scheme])) {
                    ...
                    if (!isset($options['proxy']['no']) || !\pxlrbt\Cf7Cleverreach\Vendor\GuzzleHttp\is_host_in_noproxy($host, $options['proxy']['no'])) {
        ...
        if (!empty($options['debug'])) {
            $conf[\CURLOPT_STDERR] = \pxlrbt\Cf7Cleverreach\Vendor\GuzzleHttp\debug_resource($options['debug']);
