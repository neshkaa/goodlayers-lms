...
<tr>
	<th><?php esc_html_e('Summary', 'gdlr-lms'); ?></th>
	<th align="center" ><?php esc_html_e('Scores', 'gdlr-lms'); ?></th>
</tr>
<?php 
	global $wpdb;
	...
	foreach($quiz_options as $quiz_option){
		...
		echo '<td>';
		echo empty($score_summary[$pnum])? esc_html__('Pending', 'gdlr-lms'): $score_summary[$pnum]['score'] . '/' . $score_summary[$pnum]['from'];
		echo '</td>';
		echo '</tr>';		
			...
			foreach( $quiz_option['question'] as $question ){
				...
				if( !empty($quiz_answer[$pnum][$qnum]) ){
					if( $quiz_option['question-type'] == 'single' ){
						echo gdlr_lms_text_filter($quiz_answer[$pnum][$qnum] . '.) ' . $question['quiz-choice'][$quiz_answer[$pnum][$qnum]-1]);
						...
						foreach( $quiz_answer[$pnum][$qnum] as $answer ){
							...
							echo gdlr_lms_text_filter($answer . '.) ' . $question['quiz-choice'][$answer-1]);
						}	
					}else{
