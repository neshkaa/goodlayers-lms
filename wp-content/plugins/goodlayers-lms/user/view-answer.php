<?php
	...
	$sql .= $wpdb->prepare('WHERE course_id = %d AND quiz_id = %d AND student_id= %d ', $_GET['course_id'], $_GET['quiz_id'], $current_user->ID);
	...
	if( !empty($result) ){
		...
		if( false && in_array($quiz_options[$lms_page-1]['question-type'], array('large', 'small')) ){
				...
				switch($quiz_options[$lms_page-1]['question-type']){
					...
					case 'small':
						...
						if( !empty($question['quiz-answer']) ){
							echo gdlr_lms_text_filter($question['quiz-answer']);
						}
						echo '</textarea>';
						break;
				}
