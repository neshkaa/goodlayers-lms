			...
			<div class="gdlr-lms-item">	
				<form action="" method="POST" id="payment-form" class="gdlr-lms-form" data-ajax="<?php echo admin_url('admin-ajax.php'); ?>" data-invoice="<?php echo empty($_GET['invoice'])? 0: $_GET['invoice']; ?>" >
					...
					<p class="gdlr-lms-half-left">
						<label><span><?php esc_html_e('Card Number', 'gdlr-lms'); ?></span></label>
						<input type="text" size="20" class="card-number" />
					</p>
					<div class="clear" ></div>

					<p class="gdlr-lms-half-left">
						<label><span><?php esc_html_e('CVC', 'gdlr-lms'); ?></span></label>
						<input type="text" size="4" class="card-cvc" />
					</p>
					<div class="clear" ></div>

					<p class="gdlr-lms-half-left gdlr-lms-expiration">
						<label><span><?php esc_html_e('Expiration (MM/YYYY)', 'gdlr-lms'); ?></span></label>
						<input type="text" size="2" class="card-expiry-month" />
						<span class="gdlr-separator" >/</span>
						<input type="text" size="4" class="card-expiry-year" />
					</p>
					<div class="clear" ></div>
					<div class="gdlr-lms-loading gdlr-lms-instant-payment-loading"><?php esc_html_e('loading', 'gdlr-lms'); ?></div>
					<div class="gdlr-lms-notice gdlr-lms-instant-payment-notice"></div>
					<input type="submit" class="gdlr-lms-button cyan submit-button" value="<?php esc_html_e('Submit Payment', 'gdlr-lms'); ?>" >
				</form>
			</div>
		</div>
	</div>
