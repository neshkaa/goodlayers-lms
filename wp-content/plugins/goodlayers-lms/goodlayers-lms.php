...
//	include_once('include/payment-api/stripe-php/init.php');
	...
	if( !function_exists('gdlr_lms_text_filter') ){
		add_filter('gdlr_lms_text_filter', 'do_shortcode', 11);
		function gdlr_lms_text_filter( $text ){
			return apply_filters('gdlr_lms_text_filter', $text);
		}
	}
	...
	// Custom shortcode for login/redirect functionality
// Custom shortcode for login/redirect functionality
add_shortcode('student_login', 'gdlr_lms_student_login_shortcode');
function gdlr_lms_student_login_shortcode($atts) {
    ...
    $button_style = 'color: #000; text-align: center; background-color: #fff570; border-color: #fff570; font-size: 16px; width: 50%; text-transform:uppercase;';
    
    if (is_user_logged_in()) {
        // For logged-in users, output a simple link
        echo '<div style="text-align:center"><div style="text-align: center;">';
        echo '<a class="donate-button gdlr-button with-border" style="' . $button_style . '" href="' . esc_url($atts['logged_in_link']) . '">' . esc_html($atts['text']) . '<br></a>';
        echo '</div></div>';
    } else {
        // For logged-out users, use the plugin's existing login function
        echo '<div style="text-align:center"><div style="text-align: center;">';
        echo '<a class="donate-button gdlr-button with-border" style="' . $button_style . '" data-rel="gdlr-lms-lightbox" data-lb-open="login-form">' . esc_html($atts['text']) . '<br></a>';
