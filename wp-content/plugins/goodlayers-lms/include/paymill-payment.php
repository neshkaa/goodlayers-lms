...
var PAYMILL_PUBLIC_KEY = '<?php echo gdlr_lms_text_filter($gdlr_lms_option['paymill-public-key']); ?>';
...
<?php	}
	...
	function gdlr_lms_paymill_payment(){	
		...
		if( !empty($_POST['token']) && !empty($_POST['invoice']) ){
			...
			$temp_sql .= $wpdb->prepare("WHERE id = %d", $_POST['invoice']);	
			...
			try{
				...
				$wpdb->update( $wpdb->prefix . 'gdlrpayment', 
					array('payment_status'=>'paid', 'attachment'=>serialize($response), 'payment_date'=>current_time('mysql')), 
				...
				gdlr_lms_mail($payment_info['email'], 
					esc_html__('Stripe Payment Received', 'gdlr-lms'), 
					esc_html__('Your verification code is', 'gdlr-lms') . ' ' . $payment_info['code']);				
				
				$ret['status'] = 'success';
				$ret['message'] = esc_html__('Payment complete, redirecting to the course page.', 'gdlr-lms');
			...
			$ret['status'] = 'failed';
			$ret['message'] = esc_html__('Failed to retrieve the course, please made the payment from course page again.', 'gdlr-lms');	
		}
