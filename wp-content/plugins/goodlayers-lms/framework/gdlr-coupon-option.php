<?php
	...
	*/
	...
	if( !function_exists('gdlr_lms_create_coupon') ){
		function gdlr_lms_create_coupon() {
			register_post_type( 'coupon',
				array(
					'labels' => array(
						'name'               => esc_html__('Coupon', 'gdlr-lms'),
						'singular_name'      => esc_html__('Coupon', 'gdlr-lms'),
						'add_new'            => esc_html__('Add New', 'gdlr-lms'),
						'add_new_item'       => esc_html__('Add New Coupon', 'gdlr-lms'),
						'edit_item'          => esc_html__('Edit Coupon', 'gdlr-lms'),
						'new_item'           => esc_html__('New Coupon', 'gdlr-lms'),
						'all_items'          => esc_html__('All Coupon', 'gdlr-lms'),
						'view_item'          => esc_html__('View Coupon', 'gdlr-lms'),
						'search_items'       => esc_html__('Search Coupon', 'gdlr-lms'),
						'not_found'          => esc_html__('No coupon found', 'gdlr-lms'),
						'not_found_in_trash' => esc_html__('No coupon found in Trash', 'gdlr-lms'),
						'parent_item_colon'  => '',
						'menu_name'          => esc_html__('Coupon', 'gdlr-lms')
					),
					'public'             => true,
	...
	function gdlr_lms_add_coupon_meta_box(){
		add_meta_box('course-option', esc_html__('Coupon Option', 'gdlr-lms'),
	...
	function gdlr_lms_create_coupon_meta_box(){
		...
		$coupon_settings = array(
			'coupon-code' => array(
				'title' => esc_html__('Coupon Code' , 'gdlr-lms'),
				'type' => 'text',
				'custom_field' => 'gdlr-coupon-code'
			),
			'coupon-amount' => array(
				'title' => esc_html__('Coupon Amount' , 'gdlr-lms'),
				'type' => 'text',
				'default' => -1,
				'description' => esc_html__('Fill -1 for unlimited uses', 'gdlr-lms')
			),	
			'coupon-expiry' => array(
				'title' => esc_html__('Coupon Expiry Date' , 'gdlr-lms'),
				'type' => 'datepicker'
			),
			'coupon-discount-type' => array(
				'title' => esc_html__('Coupon Discount Type' , 'gdlr-lms'),
				'type' => 'combobox',
				'options' => array(
					'percent' => esc_html__('Percent', 'gdlr-lms'),
					'amount' => esc_html__('Amount', 'gdlr-lms')
				)
			),
			'coupon-discount-amount' => array(
				'title' => esc_html__('Coupon Discount Amount' , 'gdlr-lms'),
				'type' => 'text',
				'description' => esc_html__('Only number is allowed here', 'gdlr-lms')
			),
			'specify-course' => array(
				'title' => esc_html__('Apply only to specific course ( course id separated by comma )' , 'gdlr-lms'),
				'type' => 'textarea'
			),
		);
