<?php
	...
	*/	
	...
	function gdlr_lms_add_user_role(){
		add_role('instructor', esc_html__('Instructor', 'gdlr-lms'), 
		...
		add_role('student', esc_html__('Student', 'gdlr-lms'));
	...
	add_filter('wpml_translatable_user_meta_fields', 'gdlr_lms_wpml_translatable_user_meta_fields');
	function gdlr_lms_wpml_translatable_user_meta_fields($fields){
	   $fields[] = 'author-biography';
	   $fields[] = 'phone';
	   $fields[] = 'address';
	   $fields[] = 'location';
	   $fields[] = 'position';
...
<h3><?php esc_html_e('Extra information', 'gdlr-lms'); ?></h3>
...
<table class="form-table">
	...
	<tr>
		<th><label for="author-image"><?php esc_html_e('Author Image', 'gdlr-lms'); ?></label></th>
		<td>
			<?php 
				$image_id = get_the_author_meta('author-image', $user->ID);
				$image_url = wp_get_attachment_image_src($image_id);
			?>
			<img src="<?php echo esc_attr($image_url[0]); ?>" alt="" style="max-width: 150px;" /><br />
			<input type="text" class="gdlr-lms-display regular-text" value="<?php echo esc_attr($image_url[0]); ?>" />
			<input type="hidden" class="gdlr-lms-image" name="author-image" id="author-image" value="<?php echo esc_attr($image_id); ?>" />
			<input type="button" class="gdlr-lms-upload-image" value="<?php esc_html_e('Upload', 'gdlr-lms'); ?>" data-title="<?php esc_html_e('Upload', 'gdlr-lms'); ?>" />
		</td>
	</tr>
	<tr>
		<th><label for="author-biography"><?php esc_html_e('Author Biography ( in single instructor page )', 'gdlr-lms'); ?></label></th>
		<td>
	...
	<tr>
		<th><label for="gender"><?php esc_html_e('Gender', 'gdlr-lms'); ?></label></th>
		<td><?php $value = get_the_author_meta('gender', $user->ID); ?>
			<select name="gender" id="gender">
			<option value="m" <?php echo ($value == 'm')? 'selected': ''; ?> ><?php esc_html_e('Male', 'gdlr-lms') ?></option>
			<option value="f" <?php echo ($value == 'f')? 'selected': ''; ?> ><?php esc_html_e('Female', 'gdlr-lms') ?></option>
			</select>
		</td>
	</tr>
	<tr>
		<th><label for="birth-date"><?php esc_html_e('Birth Date', 'gdlr-lms'); ?></label></th>
		<td>
	...
	<tr>
		<th><label for="phone"><?php esc_html_e('Phone', 'gdlr-lms'); ?></label></th>
		<td>
			<input type="text" name="phone" id="phone" value="<?php echo get_the_author_meta('phone', $user->ID); ?>" class="regular-text" /><br />
		</td>
	</tr>
	<tr>
		<th><label for="address"><?php esc_html_e('Address', 'gdlr-lms'); ?></label></th>
		<td>
	...
	<tr>
		<th><label for="past-work"><?php esc_html_e('Past Work', 'gdlr-lms'); ?></label></th>
		<td>
			<input type="text" name="past-work" id="past-work" value="<?php echo get_the_author_meta('past-work', $user->ID); ?>" class="regular-text" /><br />
		</td>
	</tr>	
	<tr>
		<th><label for="specialist"><?php esc_html_e('Specialist in', 'gdlr-lms'); ?></label></th>
		<td>
	...
	<tr>
		<th><label for="experience"><?php esc_html_e('Experience', 'gdlr-lms'); ?></label></th>
		<td>
	...
	<tr>
		<th><label for="social-network"><?php esc_html_e('Social Network', 'gdlr-lms'); ?></label></th>
		<td>
	...
	function gdlr_lms_author_redirect(){
		...
		add_filter('template_include', 'gdlr_lms_register_author_template', 9999);
	...
	function gdlr_lms_register_author_template($template){
		...
		if(is_author() && empty($_GET['post_type'])){
	...
	function gdlr_lms_course_search_query($query){
		if( is_search() && $query->is_main_query() && !empty($_GET['course_type']) ){
