	...
	*/	
	...
	class gdlr_lms_admin_option{
		...
		function gdlr_lms_save_admin_panel(){
			if( !check_ajax_referer('gdlr-lms-create-nonce', 'security', false) ){
				die(json_encode(array(
					'status'=>'failed', 
					'message'=> '<span class="head">' . esc_html__('Invalid Nonce', 'gdlr-lms') . '</span> ' .
						esc_html__('Please refresh the page and try this again.' ,'gdlr-lms')
				)));
			}
			...
			if( isset($_POST['option']) ){		
				...
				if($old_option == $option || update_option($this->setting['save_option'], $option)){
					$ret = array(
						'status'=> 'success', 
						'message'=> '<span class="head">' . esc_html__('Save Options Complete' ,'gdlr-lms') . '</span> '
					);		
				}else{
					$ret = array(
						'status'=> 'failed', 
						'message'=> '<span class="head">' . esc_html__('Save Options Failed', 'gdlr-lms') . '</span> ' .
						esc_html__('Please refresh the page and try this again.' ,'gdlr-lms')
					);					
				}
			}else{
				$ret = array(
					'status'=>'failed', 
					'message'=> '<span class="head">' . esc_html__('Cannot Retrieve Options', 'gdlr-lms') . '</span> ' .
						esc_html__('Please refresh the page and try this again.' ,'gdlr-lms')
				);	
			}
			...
			foreach( $this->option as $menu_slug => $menu_settings ){
				...
				foreach( $menu_settings['options'] as $sub_menu_slug => $sub_menu_settings ){
					if( !empty($sub_menu_settings) ){
						...
						echo '<div class="sub-menu-title">' . $sub_menu_settings['title'] . '</div>';
						echo '</li>';
		...
		function print_admin_content(){
			...
			echo '<input value="' . esc_html__('Save Changes', 'gdlr-lms') . '" type="submit" class="gdl-button" />';
			echo '</div>'; 
