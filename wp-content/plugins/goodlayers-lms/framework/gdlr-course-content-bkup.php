	...
	*/
	...
	if( !function_exists('gdlr_lms_init_course_bkup') ){
		function gdlr_lms_init_course_bkup(){
			add_meta_box( 'course-bkup-option', 
				esc_html__('Revert To Last Working Data', 'good-lms'), 
	...
	if( !function_exists('gdlr_lms_create_course_bkup_option') ){
		function gdlr_lms_create_course_bkup_option(){
			...
			echo '<em>';
			echo esc_html__('*This option helps you restore the last working data when the content disappear. Note that to use this option will replace all your current page item setting in this page and <strong>This Can\'t Be Undone</strong>.', 'good-lms');
			echo '</em><div class="clear"></div>';
			echo '<input type="button" value="' . esc_html__('Restore Lost Data', 'good-lms') . '" />';
			echo '</div>';
