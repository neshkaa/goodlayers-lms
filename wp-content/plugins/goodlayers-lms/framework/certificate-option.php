	...
	*/	
	...
	function gdlr_lms_create_certificate() {
		register_post_type( 'certificate',
			array(
				'labels' => array(
					'name'               => esc_html__('Certificate', 'gdlr-lms'),
					'singular_name'      => esc_html__('Certificate', 'gdlr-lms'),
					'add_new'            => esc_html__('Add New', 'gdlr-lms'),
					'add_new_item'       => esc_html__('Add New Certificate', 'gdlr-lms'),
					'edit_item'          => esc_html__('Edit Certificate', 'gdlr-lms'),
					'new_item'           => esc_html__('New Certificate', 'gdlr-lms'),
					'all_items'          => esc_html__('All Certificates', 'gdlr-lms'),
					'view_item'          => esc_html__('View Certificate', 'gdlr-lms'),
					'search_items'       => esc_html__('Search Certificate', 'gdlr-lms'),
					'not_found'          => esc_html__('No certificates found', 'gdlr-lms'),
					'not_found_in_trash' => esc_html__('No certificates found in Trash', 'gdlr-lms'),
					'parent_item_colon'  => '',
					'menu_name'          => esc_html__('Certificates', 'gdlr-lms')
				),
				'public'             => true,
	...
	function gdlr_lms_add_certificate_meta_box(){
		add_meta_box('certificate-option', esc_html__('Certificate Option', 'gdlr-lms'), 
	...
	function gdlr_lms_create_certificate_meta_box(){
		...
		$certificate_settings = array(
			...
			'custom-css' => array(
				'title' => esc_html__('Custom Certificate Css', 'gdlr-lms'),
				'type' => 'textarea'
			),
			'fill-default' => array(
				'title' => esc_html__('Fill Default Shortcode Text', 'gdlr-lms'),
				'type' => 'button',
				'default' => esc_html__('Fill Shortcode', 'gdlr-lms')
			),
			'shortcode-description' => array(
				'title' => esc_html__('Shortcode Description', 'gdlr-lms'),
				'type' => 'description',
				'default' => 
...
'<br><span class="head">Signature - </span>[gdlr_cer_signature image="XXX" font_size="15px" margin_top="0px" class="XXX"]Sam White, Course Instructor[/gdlr_cer_signature]'
		...
		echo '<h3>' . esc_html__('Certificate Settings', 'gdlr-lms') . '</h3>';
