...
?>
<div class="wrap">
<h2><?php esc_html_e('Instructor Commission', 'gdlr-lms'); ?></h2>
...
<?php 
	...
	foreach($results as $result){
		...
		echo '<td>';
		echo '<a data-rel="gdlr-lms-lightbox" data-lb-open="commission-rate">' . esc_html__('Change % Rate', 'gdlr-lms') . '</a>';
...
<div class="gdlr-lms-lightbox-container light commission-rate">	
	<h3 class="gdlr-lms-lightbox-title"><?php echo esc_html__('Commission Rate (%)', 'gdlr-lms'); ?></h3>
	<form class="gdlr-lms-form gdlr-lms-lightbox-form" method="get">
		<input type="text" name="commission_rate" value="<?php echo esc_attr($rate); ?>" />
		...
		<input type="submit" class="gdlr-lms-button" value="<?php esc_html_e('Update Rate', 'gdlr-lms'); ?>" />
		<div class="clear"></div>
	</form>
</div>
<?php	
