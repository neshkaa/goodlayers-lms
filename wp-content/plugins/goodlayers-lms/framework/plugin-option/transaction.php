	...
	function gdlr_lms_transaction_option(){
		...
		if( !empty($_POST['action']) && !empty($_POST['tid']) ){
			...
			if($_POST['action'] == 'delete'){
				...
				$sql .= $wpdb->prepare('WHERE id = %d', $_POST['tid']);
...
?>
<div class="wrap">
<h2><?php esc_html_e('Transaction List', 'gdlr-lms'); ?></h2>
<form class="gdlr-lms-transaction-form" method="GET" action="">
	<div class="gdlr-lms-transaction-form-row">
		<span class="gdlr-lms-head"><?php esc_html_e('Search transaction by :', 'gdlr-lms'); ?></span>
		<div class="gdlr-combobox-wrapper">
		<select name="selector" >
			<option value="name" <?php echo (!empty($_GET['selector']) && $_GET['selector']=='name')? 'selected': ''; ?> ><?php esc_html_e('Name', 'gdlr-lms'); ?></option>
			<option value="code" <?php echo (!empty($_GET['selector']) &&$_GET['selector']=='code')? 'selected': ''; ?> ><?php esc_html_e('Code', 'gdlr-lms'); ?></option>
		</select>
		</div>
	...
	<div class="gdlr-lms-transaction-form-row">
		<span class="gdlr-lms-head"><?php esc_html_e('Include transaction that has price = 0 :', 'gdlr-lms'); ?></span>
		<input type="checkbox" name="price_zero" value="1" <?php echo !empty($_GET['price_zero'])? 'checked': ''; ?> style="margin-top: 8px;" />
		<div class="clear"></div>
	</div>
	<div class="gdlr-lms-transaction-form-row">
		<span class="gdlr-lms-head"><?php esc_html_e('Keywords :', 'gdlr-lms'); ?></span>
		...
		<input type="submit" value="<?php esc_html_e('Search!', 'gdlr-lms'); ?>" />
		<div class="clear"></div>
	</div>
</form>
...
<tr>
	<th><?php esc_html_e('ID', 'gdlr-lms'); ?></th>
	<th><?php esc_html_e('Name', 'gdlr-lms'); ?></th>
	<th><?php esc_html_e('Course', 'gdlr-lms'); ?></th>
	<th><?php esc_html_e('Type', 'gdlr-lms'); ?></th>
	<th><?php esc_html_e('Price', 'gdlr-lms'); ?></th>
	<th><?php esc_html_e('Status', 'gdlr-lms'); ?></th>
	<th><?php esc_html_e('Code', 'gdlr-lms'); ?></th>
	<th><?php esc_html_e('Booked/Paid Date', 'gdlr-lms'); ?></th>
	<th><?php esc_html_e('Action', 'gdlr-lms'); ?></th>
</tr>
<?php
	...
	if( !empty($_GET['selector']) && !empty($_GET['keywords']) ){
		if( $_GET['selector'] == 'name' ){
			...
			$temp_sql .= $wpdb->prepare('AND payment_info LIKE %s ', '%' . $_GET['keywords'] . '%');
		}
	...
	for($i=($record_per_page*($current_page - 1)); $i<$record_num && $i<($record_per_page * $current_page); $i++){ $result = $results[$i];
		...
		echo '<td>';
		echo ($course_options['online-course']=='enable')? esc_html__('Online', 'gdlr-lms'): esc_html__('Onsite', 'gdlr-lms');
		echo '</td>';
		echo '<td>' . gdlr_lms_money_format(number_format_i18n($result->price, 2)) . '</td>';
		echo '<td>';
		if( $result->payment_status == 'paid' ){
			echo esc_html__('paid', 'gdlr-lms');
		...
		echo '<td>';
		if( $result->payment_status == 'paid' ){
			echo '<a class="mark-as-pending" data-id="' . $result->id . '" >' . esc_html__('Mark As Pending', 'gdlr-lms') . '</a>';
		}else{
			echo '<a class="mark-as-paid" data-id="' . $result->id . '" >' . esc_html__('Mark As Paid', 'gdlr-lms') . '</a>';
		}
		echo '<span class="gdlr-separator">/</span>';
		echo '<a class="delete-transaction" data-id="' . $result->id . '" >' . esc_html__('Delete', 'gdlr-lms') . '</a>';
		echo '</td>';
		echo '</tr>';
	}
?>
...
<?php
	...
	if( $max_num_page > 1 ){
		...
		if($current_page > 1){
			...
			echo esc_html__('&lsaquo; Previous', 'gdlr-lms') . '</a>';
		}
		...
		if($current_page < $max_num_page){
			...
			echo esc_html__('Next &rsaquo;', 'gdlr-lms') . '</a>';
		}
		echo '</div>';
	}
?>
