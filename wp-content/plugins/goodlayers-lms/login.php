...
if( isset($_GET['action']) && $_GET['action'] == 'reset_pass' ){
	...
	if( isset($_POST['pass1']) && $_POST['pass1'] != $_POST['pass2'] ){
		$rp_errors->add('password_reset_mismatch', esc_html__('The passwords do not match.', 'gdlr-lms'));
	}
...
<?php if( isset($_GET['action']) && $_GET['action'] == 'lost_password' ){ ?>
	<?php
		if( empty($_GET['status']) ){
			...
			esc_html_e('Please enter your username or email address. You will receive a link to create a new password via email.', 'gdlr-lms');
			echo '</div>';
		}else{
			echo '<div class="gdlr-lms-error">';
			if( $_GET['status'] == 'no_password_reset' ){
				esc_html_e('Password reset is not allowed for this user', 'gdlr-lms');
			}else if( $_GET['status'] == 'expiredkey' ){
				esc_html_e('Sorry, that key has expired. Please try again.', 'gdlr-lms');
			}else if( $_GET['status'] == 'invalidkey' ){
				esc_html_e('Sorry, that key does not appear to be valid.', 'gdlr-lms');
			}else{
				esc_html_e('Invalid username or e-mail.', 'gdlr-lms');
			}
			echo '</div>';
		}
	?>
	...
	<form name="lostpasswordform" class="gdlr-lms-form" action="<?php echo esc_url( network_site_url( 'wp-login.php?action=lostpassword', 'login_post' ) ); ?>" method="post">
		<p class="gdlr-lms-half-left">
			<label><?php esc_html_e('Username or E-mail:') ?></label>
			<input type="text" name="user_login" class="input" value="<?php echo esc_attr($user_login); ?>" size="20" />
		</p>
		<div class="clear"></div>
		<?php do_action( 'lostpassword_form' ); ?>
		<p>
			<input type="hidden" name="login" value="<?php echo ($_GET['login'] == 'home')? home_url('/'): $_GET['login']; ?>" />
			<input type="submit" class="gdlr-lms-button" value="<?php esc_html_e('Get New Password', 'gdlr-lms'); ?>" />
		</p>
	</form>
...
<?php }else if( isset($_GET['action']) && $_GET['action'] == 'reset_pass' ){ 
	if( is_wp_error($rp_errors) && $rp_errors->get_error_code() ){
		echo '<div class="gdlr-lms-error">' . $rp_errors->get_error_message() . '</div>';
	}
?>

<form class="gdlr-lms-form" method="post" >
	<p>
		<?php esc_html_e('<strong>Hint:</strong> The password should be at least seven characters long. To make it stronger, use upper and lower case letters, numbers, and symbols like ! " ? $ % ^ &amp; ).', 'gdlr-lms'); ?>
	</p>
	...
	<p>
		<input type="submit" class="gdlr-lms-button" value="<?php esc_html_e('Reset Password', 'gdlr-lms'); ?>" />
	</p>
</form>
...
<?php }else{ ?>
	<?php if(!empty($_GET['status']) && $_GET['status'] == 'login_incorrect'){ ?>
		...
		<?php esc_html_e('The login credentials is incorrect. Please try again.', 'gdlr-lms'); ?>
		</div>
	<?php }else if(!empty($_GET['status']) && $_GET['status'] == 'forgot_password_confimation'){ ?>
		<div class="gdlr-lms-success">
		<?php esc_html_e('Please check your e-mail for the confirmation link.', 'gdlr-lms'); ?>
		</div>
	<?php }else if(!empty($_GET['status']) && $_GET['status'] == 'rp_complete'){ ?>
		<div class="gdlr-lms-success">
		<?php esc_html_e('Your password has been reset.', 'gdlr-lms'); ?>
		</div>
	<?php } ?>
	...
	<form class="gdlr-lms-form" id="loginform" method="post" action="<?php echo esc_url(site_url('wp-login.php', 'login_post')); ?>">
		...
		<p class="gdlr-lms-lost-password" >
			<?php $login_url = (empty($_GET['login']) || $_GET['login'] == 'home')? home_url(): $_GET['login']; ?>
		...
		<p>
			...
			<input type="hidden" name="redirect_to" value="<?php echo esc_attr($login_url); ?>" />
