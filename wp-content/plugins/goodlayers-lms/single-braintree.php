...
<div id="content" class="site-content" role="main">
	<?php
		...
		$temp_sql .= $wpdb->prepare("WHERE id = %d", $_GET['invoice']);	
			...
			<div class="gdlr-lms-item">	
				...
				<?php if( empty($_GET['step']) || $_GET['step'] == 1 ){ ?>
					<form method="post" action="<?php echo esc_url(add_query_arg(array('step'=>2))); ?>">
					  ...
					  <input type="submit" value="<?php echo sprintf(esc_html__('Pay %s', 'gdlr-lms'), gdlr_lms_money_format(number_format_i18n($result->price, 2))); ?>">
					</form>
				...
				<?php }else if( empty($_GET['step']) || $_GET['step'] == 2 ){ 
					...
					if( !$payment->success ){
						...
						esc_html__('Please try again', 'gdlr-lms');
						echo '</div>';
						echo '<a class="gdlr-lms-button blue" href="' . get_permalink($result->course_id) . '" >' . esc_html__('Back To Course', 'gdlr-lms') . '</a>';
					}else{
						$payment_info = unserialize($result->payment_info);
						$wpdb->update( $wpdb->prefix . 'gdlrpayment', 
							array('payment_status'=>'paid', 'attachment'=>serialize($payment), 'payment_date'=>current_time('mysql')), 
						...
						gdlr_lms_mail($payment_info['email'], 
							esc_html__('Braintree Payment Received', 'gdlr-lms'), 
							esc_html__('Your verification code is', 'gdlr-lms') . ' ' . $payment_info['code']);		
						
						echo '<div class="gdlr-lms-success">';
						echo esc_html__('Your payment has been successfully processed. Thank you.', 'gdlr-lms');
						echo '</div>';
						echo '<a class="gdlr-lms-button blue" href="' . get_permalink($result->course_id) . '" >' . esc_html__('Back To Course', 'gdlr-lms') . '</a>';
					}
				} ?>
