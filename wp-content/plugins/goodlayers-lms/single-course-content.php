	...
	<div class="gdlr-lms-content">
		...
		<?php 
			...
			while( have_posts() ){ the_post();
				...
				echo '<div class="gdlr-lms-course-info-title">' . esc_html__('Course Process', 'gdlr-lms') . '</div>';
				...
				for( $i=1; $i<=sizeof($gdlr_course_content); $i++ ){
					...
					if( strpos($part_class, 'gdlr-current') !== false && 
						...
						for( $j=1; $j<=sizeOf($lectures); $j++  ){
							...
							echo '<span class="lecture-part">' . sprintf(esc_html__('Lecture %d', 'gdlr-lms'), ($prev_lnum + $j)) . '</span>';
