			...
			while( have_posts() ){ the_post();
				...
				for( $i=1; $i<=sizeof($gdlr_course_content); $i++ ){
					...
					echo '<div class="gdlr-lms-course-part-content">';
					    ...
					    // Check if section name is "Подготовка" for custom link
    if($gdlr_course_content[$i-1]['section-name'] === 'Идентификация & подготовка') {
        echo '<a href="/kak-da-kastriram/identifikacia/">';
    } else {
    ...
    echo '</a>';
					...
					if( strpos($part_class, 'gdlr-current') !== false && 
						...
						for( $j=1; $j<=sizeOf($lectures); $j++  ){
							...
							echo '<div class="gdlr-lms-lecture-part-content">';
						            ...
						            // Check if section name is "Подготовка" AND lecture name is "Опознай котките"
              if($gdlr_course_content[$lms_page-1]['section-name'] === 'Идентификация & подготовка' && 
               !empty($lectures[$j-1]['lecture-name']) && 
               $lectures[$j-1]['lecture-name'] === 'Опознай котките') {
                echo '<a href="/kak-da-kastriram/identifikacia/opoznai-kotkite/">';
              } else if($gdlr_course_content[$lms_page-1]['section-name'] === 'Идентификация & подготовка' && 
               !empty($lectures[$j-1]['lecture-name']) && 
               $lectures[$j-1]['lecture-name'] === 'Имат ли маркировка?') {
                echo '<a href="/kak-da-kastriram/identifikacia/imat-li-markirovka/">';
              } else if($gdlr_course_content[$lms_page-1]['section-name'] === 'Идентификация & подготовка' && 
               !empty($lectures[$j-1]['lecture-name']) && 
               $lectures[$j-1]['lecture-name'] === 'Кварталните котаци') {
                echo '<a href="/kak-da-kastriram/identifikacia/kvartalnite-kotaci/">';
              } else if($gdlr_course_content[$lms_page-1]['section-name'] === 'Идентификация & подготовка' && 
               !empty($lectures[$j-1]['lecture-name']) && 
               $lectures[$j-1]['lecture-name'] === 'Пол и възраст за кастрация') {
                echo '<a href="/kak-da-kastriram/identifikacia/pol-i-vuzrast/">';
              } else if($gdlr_course_content[$lms_page-1]['section-name'] === 'Идентификация & подготовка' && 
               !empty($lectures[$j-1]['lecture-name']) && 
               $lectures[$j-1]['lecture-name'] === 'Бременни котки') {
                echo '<a href="/kak-da-kastriram/identifikacia/bremenni-kotki/">';
              } else if($gdlr_course_content[$lms_page-1]['section-name'] === 'Идентификация & подготовка' && 
               !empty($lectures[$j-1]['lecture-name']) && 
               $lectures[$j-1]['lecture-name'] === 'Кърмещи майки с котенца') {
                echo '<a href="/kak-da-kastriram/identifikacia/kurmeshti-maiki/">';
            } else {
            ...
            if( !empty($lectures[$j-1]['lecture-name']) ){
				...
				if( empty($gdlr_time_left) ){
					...
					for( $i=1; $i<=$lms_lecture; $i++ ){
						if( !empty($lectures[$i-1]['pdf-download-link']) ){
							...
							echo '<div class="gdlr-lms-part-title">' . $i . '.</div>';
				...
				if( $lms_page > 1 || ($lms_page == 1 && $lms_lecture > 1) ){
					...
					echo 'ПРЕДИШНА: ' . esc_html($lectures[$j-1]['lecture-name']);
					echo '</a>';
				}
