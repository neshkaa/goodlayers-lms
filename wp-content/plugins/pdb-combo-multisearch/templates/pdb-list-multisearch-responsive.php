...
<div class="pdb-list list-container" >
      ...
      <h5 class="search-result-message" ><?php printf( _n( 'Found %d record', 'Found %d records', $record_count, 'pdb-combo-multisearch' ), $record_count ) ?></h5>
    <?php elseif( !empty( $search_term ) ) : ?>
      <h5 class="search-result-message" ><?php printf( _n(  'Found %d result while searching for %s.', 'Found %d results while searching for %s.', $record_count, 'pdb-combo-multisearch' ), $record_count, '<strong>' . $search_term . '</strong>' ) ?></h5>
      <?php else: ?>
      <?php $this->print_list_count( '<h5 class="search-result-message %s"><span class="list-display-count">' ) ?>
    <?php endif ?>
    ...
    <?php while ( $this->have_records() ) : $this->the_record(); // each record is one row ?>
      <section id="record-<?php echo $this->record->record_id ?>">
        ...
        <div class="pdb-field <?php echo $this->field->name() ?>-field">
          <span class="pdb-field-title"><?php echo $this->field->print_label() ?></span>
