...
<div class="wrap <?php echo $this->wrap_class ?> pdb-combo-multisearch" id="<?php echo $this->list_anchor ?>">
  ...
  <table class="wp-list-table widefat fixed pages list-container" cellspacing="0" >
    <?php if ( empty( $search_term ) && $PDb_Combo_Multi_Search->is_search_result() ) : ?>
      <caption class="search-result-message"><?php printf( _n( 'Found %d record', 'Found %d records', $record_count, 'pdb-combo-multisearch' ), $record_count ) ?></caption>
    <?php elseif ( !empty( $search_term ) ) : ?>
      <caption class="search-result-message"><?php printf( _n( 'Found %d result while searching for %s.', 'Found %d results while searching for %s.', $record_count, 'pdb-combo-multisearch' ), $record_count, '<strong>' . $search_term . '</strong>' ) ?></caption>
    <?php else: ?>
      <?php $this->print_list_count( '<caption class="search-result-message %s" ><span class="list-display-count">' ) ?>
    <?php endif ?>
